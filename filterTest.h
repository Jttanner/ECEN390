...
/****************************************************************************************************
 * Comment out the line below to perform a filter-only verification. Leave the line uncommented and
 * you perform the verification from the adcBuffer to the detector, for a more thorough test.
 * Make sure to leave the line below commented until you have completely coded detector.c and isr.c
 * Otherwise this file will not compile.
 ***************************************************************************************************/
...
/****************************************************************************************************
 * Uncomment the line below if you are using output queues of size 2001 to incrementally compute power.
 * In this case, the oldest value is located at location 0 in the output queue, and
 * you compute the current power from 1 to 1999 (all output-queue values except value 0).
 * The default is to use a queue-size of 2000.
 ****************************************************************************************************/
...
// Testing frequencies include the 10 user frequencies and some number of "out of band" frequencies.
#define FILTER_TEST_FIR_POWER_TEST_PERIOD_COUNT (FILTER_FREQUENCY_COUNT + FILTER_TEST_OUT_OF_BAND_TICK_COUNT)
// Out of band frequencies defined similar to user frequencies, as tick counts. All are square waves.
static const uint16_t filterTest_firTestOutOfBandTickCounts[FILTER_TEST_OUT_OF_BAND_TICK_COUNT] = {22, 20, 18, 16, 14, 12, 10, 8, 6, 4, 2};
#define MAX_BUF 10                            // Used for a temporary char buffer.
#define FILTER_TEST_PULSE_WIDTH_LENGTH 20000  // The pulse-width is 20000 because everything in the test runs at 100 kHz.
#define FILTER_TEST_MIN_INPUT_VALUE (-1.0)    // This is the top of the square wave.
#define FILTER_TEST_MAX_INPUT_VALUE (1.0)     // This is the bottom of the square wave.
#define FILTER_TEST_INPUT_OFFSET (1.0)        // Add this to the input if you need to make it unipolar (for plotting the input).

// This array will contain the frequency tick counts that will be applied against the FIR filter.
// User frequencies + out-of-band frequencies.
static uint16_t filterTest_firTestTickCounts[FILTER_TEST_OUT_OF_BAND_TICK_COUNT + FILTER_FREQUENCY_COUNT];

/******************************************************************************************
 ******************************* Start of functions ***************************************
 ******************************************************************************************/

static bool filterTest_initFlag = false;  // True if the filterTest_init() function was called.
...
void filterTest_init() {
    ...
    for (uint16_t j=FILTER_FREQUENCY_COUNT; j<FILTER_FREQUENCY_COUNT+FILTER_TEST_OUT_OF_BAND_TICK_COUNT; j++) {
        filterTest_firTestTickCounts[j] = filterTest_firTestOutOfBandTickCounts[j-FILTER_FREQUENCY_COUNT];
    }
...
queue_data_t filterTest_readMostRecentValueFromQueue(queue_t* q) {
    if (queue_empty(q))
        ...
        return queue_readElementAt(q, queue_elementCount(q)-1); // Rely on queue_readElementAt().
}
...
// 1. The size of the array is FILTER_FIR_POWER_TEST_PERIOD_COUNT and it contains power for these tested frequencies.
...
// This assumes that the TFT display is in mode 1 (landscape-mode with the origin at the top-left).
// Incoming data are assumed unipolar: 0 to some max value
// Incoming data are scaled in X and Y and mapped so that the plotted range is: -1.0 to 1.0.
// Negative values are plotted on the bottom half of the screen, positive values are plotted on the top-half.
// A single horizontal line marks the y-value of 0.
...
double computeFilterInput(uint16_t freqTick, uint16_t currentPeriodTickCount) {
    ...
    if (freqTick < ONE_HALF(currentPeriodTickCount))// The period has two halves: the -1.0 part and the 1.0 part.
        filterValue = FILTER_TEST_MIN_INPUT_VALUE; // The first half of the period is a minimum value.
    else
        filterValue = FILTER_TEST_MAX_INPUT_VALUE; // The second half of the period is a maximum value.
    return filterValue;
}

#ifdef ADC_THROUGH_DETECTOR_FILTER_TEST
// When performing a complete system test, the input goes to the ADC buffer and must be computed accordingly.
uint16_t computeAdcBufferInput(uint16_t freqTick, uint16_t currentPeriodTickCount) {
  uint16_t adcValue;
  //  if (freqTick < currentPeriodTickCount/2)// The period has two halves: the -1.0 part and the 1.0 part.
  if (freqTick < ONE_HALF(currentPeriodTickCount)// The period has two halves: the -1.0 part and the 1.0 part.
...
// Output values are retrieved from a FIR debug queue (see filter_getFirOutputDebugQueue()).
// Power is computed internally. Does not use the filter_computePower... functions.
// To plot the input as well as output, pass true to plotInputFlag.
#define PLOT_VALUE_MAX_COUNT 360  // You can collect up to this many values to plot.
#define PERIODS_TO_PLOT 2         // The number of period's worth of data to collect and plot.
  ...
  isr_init();       // You will be using adcBuffer to provide data to the filters.
    ...
    if (printMessageFlag) {
        ...
        printf("running filter_runFirPowerTest() - plotting power values (frequency response) for frequencies %1.2lf kHz to %1.2lf kHz for FIR filter to TFT display.\n\r",
               ((double) ((FILTER_SAMPLE_FREQUENCY_IN_KHZ))/filterTest_firTestTickCounts[0]),
               ((double) ((FILTER_SAMPLE_FREQUENCY_IN_KHZ))/filterTest_firTestTickCounts[FILTER_TEST_FIR_POWER_TEST_PERIOD_COUNT-1]));
    }
    double testPeriodPowerValue[FILTER_TEST_FIR_POWER_TEST_PERIOD_COUNT];  // Computed power values will go here.
    ...
    for (uint16_t testPeriodIndex=0; testPeriodIndex<FILTER_TEST_FIR_POWER_TEST_PERIOD_COUNT; testPeriodIndex++) {
        firPower = 0.0; // Reset the power value.
        uint16_t currentPeriodTickCount = filterTest_firTestTickCounts[testPeriodIndex];
        uint32_t totalTickCount = 0;  // Keep track of where you are in the period.
        // The #ifdef below allow the code to adapt to early usage when only the filter.c code is being tested up through
        // late-end verification when the isr.c, detector.c and so forth have been coded and are being tested.
        ...
        isr_addDataToAdcBuffer(adcValue);       // Put the data into the adcBuffer.
                ...
                filter_addNewInput(filterValue);       // Put the data into the input queue of the filter.
                if (filterTest_decimatingFirFilter()) {
                    double firOutput = filterTest_readMostRecentValueFromQueue(filter_getYQueue()); // Get the output from the yQueue.
                    firPower += firOutput * firOutput; // Compute the power so far.
                }
#endif
                if (plotInputFlag && (totalTickCount < currentPeriodTickCount*PERIODS_TO_PLOT)) { // You can capture multiple periods of data.
                    xValues[totalTickCount] = totalTickCount; // You just need x to increment.
#ifdef ADC_THROUGH_DETECTOR_FILTER_TEST
                    yValues[totalTickCount] = adcValue;       // y is the adcValue.
#else
                    yValues[totalTickCount] = filterValue + FILTER_TEST_INPUT_OFFSET;    // The offset ensure that the input is unipolar for plotting purposes.
#endif
                }
                totalTickCount++;   // This keeps track of the total ticks in the pulse-width.
            }
        }
    ...
    detector(interruptsEnabled, false); // Run the detector so that it runs the decimating FIR and IIR filters.
    for (queue_index_t i=0; i<queue_elementCount(filter_getFirOutputDebugQueue()); i++) { // Iterate over the FIR output debug queue.
      double firOutput = queue_readElementAt(filter_getFirOutputDebugQueue(), i);         // Read an output.
      firPower += firOutput * firOutput;                                                  // Square the output.
    }
#endif
        testPeriodPowerValue[testPeriodIndex] = firPower;                                     // Store the resulting power.
        printf("freqCount:%d, testPeriodPowerValue:%le\n\r", freqCount, testPeriodPowerValue[testPeriodIndex]); // Info. print.
        freqCount++;
    }
...
// Plots the output power for a given filter across the standard 10 user frequencies.
// iirPowerValues[] contains the computed power for iir-filter(filterNumber) for all 10 user frequencies.
// Histogram bars are drawn in red and blue. Red bars represent frequencies where you want a minimal response.
// Blue histogram bars represent frequencies where you want a maximal response (when filterNumber matches the user-frequency).
...
void filterTest_plotIirFrequencyResponse(double iirPowerValues[], uint16_t filterNumber) {
    ...
    for (uint16_t barIndex=0; barIndex<FILTER_FREQUENCY_COUNT; barIndex++) {
        ...
        if (snprintf(label, HISTOGRAM_BAR_TOP_MAX_LABEL_WIDTH_IN_CHARS, "%0.0e", floor(iirPowerValues[barIndex])+ONE_HALF_FP(1.0)) == -1)
...
// Plots frequency response for the selected filterNumber against the 10 standard frequencies (square-wave).
// Plots the IIR power for a specific filterNumber for the supplied iirPowerValues.
// IIR outputs are retrieved via filter_getIirOutputQueue(filterNumber).
// Power is computed internally. Does not use the filter_compute... functions to compute power.
#define FILTER_IIR_POWER_TEST_PERIOD_COUNT FILTER_FREQUENCY_COUNT
void filterTest_runSquareWaveIirPowerTest(uint16_t filterNumber, bool printMessageFlag) {
    if (!filterTest_initFlag) {
        printf("Must call filterTest_init() before running any filter tests.\n\r");
        return;
    }
  ...
  isr_init();       // You will be using adcBuffer to provide data to the detector.
    ...
    for (uint16_t testPeriodIndex=0; testPeriodIndex<FILTER_FREQUENCY_COUNT; testPeriodIndex++) {  // Only use the first 10 standard frequencies.
        ...
        filterTest_fillQueue(filter_getZQueue(filterNumber), 0.0);  // zero out the z-queue for the IIR filter under test.
        uint16_t currentPeriodTickCount = filterTest_firTestTickCounts[testPeriodIndex];              // You will be generating a frequency with this period.
        ...
        isr_addDataToAdcBuffer(adcValue);       // Put the data into the adcBuffer.
                ...
                double iirOutput;   // This will hold the latest output from the iir filter.
                filterValue = computeFilterInput(freqTick, currentPeriodTickCount); // Compute a filter value.
                filter_addNewInput(filterValue);       // Put the data into the input queue of the filter.
                if (filterTest_decimatingFirFilter()) {  // Run the IIR filter if the fir-filter ran.
                    filter_iirFilter(filterNumber);
                    // Get the latest output from the iir-filter.
                    iirOutput = filterTest_readMostRecentValueFromQueue(filter_getZQueue(filterNumber));
                    power += iirOutput * iirOutput; // Multiply-accumulate the iir-filter output
                }
#endif
        ...
        bool interruptsEnabled = false;     // Need to tell the detector that interrupts are not currently enabled.
    detector(interruptsEnabled, false); // Run the detector so that it runs the decimating FIR and IIR filters.
    for (queue_index_t i=0; i<queue_elementCount(filter_getIirOutputQueue(filterNumber)); i++) { // Iterate over the IIR output debug queue.
      double iirOutput = queue_readElementAt(filter_getIirOutputQueue(filterNumber), i);  // Read an output.
      power += iirOutput * iirOutput;                                                     // Square the output.
    }
#endif
        testPeriodPowerValue[testPeriodIndex] = power;  // keep track of the power for each frequency.
        freqCount++;  // Next frequency.
    }
    filterTest_plotIirFrequencyResponse(testPeriodPowerValue, filterNumber);  // Finally, plot the results.
}
...
// This is equivalent to passing the filter over an input containing only a delta function and thus returns the impulse response.
bool filterTest_runFirAlignmentTest(bool printMessageFlag) {
    ...
    for (uint32_t i=0; i<filter_getFirCoefficientCount(); i++) {  // Push the single 1.0 through the queue.
        double firValue = filter_firFilter();               // Run the FIR filter.
        double firGoldenOutput =  filter_getFirCoefficientArray()[i]; // Golden output is simply the FIR coefficient.
        if (!filterTest_floatingPointEqual(firValue, firGoldenOutput)) {  // If the output from the FIR filter does not match the golden value, print an error.
            ...
            printf("filter_runAlignmentTest: Output from FIR Filter(%20.24le) does not match test-data(%20.24le).\n\r", firValue, firGoldenOutput);
        }
...
bool filterTest_runFirArithmeticTest(bool printMessageFlag) {
    ...
    for (uint32_t i=0; i<filter_getFirCoefficientCount(); i++) {  // Loop enough times to go through the coefficients.
        double newTestInput = 1.0;                          // Only value in the xQueue is 1.0.
        filter_addNewInput(newTestInput);                   // Add a 1.0 in the xQueue.
        double firValue = filter_firFilter();               // Run the FIR filter.
        firGoldenOutput += newTestInput * filter_getFirCoefficientArray()[i]; // Golden output is simply the coefficient.
        if (!filterTest_floatingPointEqual(firValue, firGoldenOutput)) {  // Test fails if the FIR output does not match the computed golden value. Print message to note failure.
            ...
            printf("filter_runArithmeticTest: Output from FIR Filter(%24.20le) does not match test-data(%24.20le) at index(%ld).\n\r", firValue, firGoldenOutput, i);
        }
    }
    // Print informational messages.
    if (printMessageFlag) {
        ...
        if (success)
            ...
            printf("failed.\n\r");
            printf("Ensure that your FIR coefficients have at least 20 significant digits.\n\r");
        }
    }
...
bool filterTest_runIirBAlignmentTest(uint16_t filterNumber, bool printMessageFlag) {
    ...
    filter_fillQueue(filter_getZQueue(filterNumber), 0.0); // zero out the zQueue for filterNumber.
    queue_overwritePush(filter_getYQueue(), 1.0);          // Place a single 1.0 in the yQueue.
    for (uint32_t i=0; i<filter_getIirBCoefficientCount(); i++) {
        double iirValue = filter_iirFilter(filterNumber);                           // Run the IIR filter.
        double iirGoldenOutput =  filter_getIirBCoefficientArray(filterNumber)[i];  // Golden output is simply the coefficient.
        if (!filterTest_floatingPointEqual(iirValue, iirGoldenOutput)) {            // Make sure they IIR output matches the golden output.
            ...
            printf("filter_runIirBlignmentTest: Output from IIR Filter[%d](%24.20le) does not match test-data(%24.20le) at index(%ld).\n\r", filterNumber, iirValue, iirGoldenOutput, i);
        }
        filter_fillQueue(filter_getZQueue(filterNumber), 0.0); // zero out the zQueue for filterNumber so the A-summation is always 0.
        queue_overwritePush(filter_getYQueue(), 0.0);          // Shift the 1.0 over one position in the yQueue.
    }
    // Print informational messages.
    if (printMessageFlag) {
...
bool filterTest_runIirAAlignmentTest(uint16_t filterNumber, bool printMessageFlag) {
    ...
    bool success = true;            // Be optimistic.
    filter_fillQueue(filter_getYQueue(), 0.0); // zero-out the yQueue so the B-summation is always 0.
    uint16_t startingIndex = filterTest_getIirACoefficientArrayStartingIndex();     // Varies according to student.
    for (uint32_t i=0; i<filter_getIirACoefficientCount()-startingIndex; i++) { // Loop through all of the A-coefficients.
        filter_fillQueue(filter_getZQueue(filterNumber), 0.0);                    // zero out the zQueue for filterNumber.
        queue_overwritePush(filter_getZQueue(filterNumber), 1.0);                 // Add a single 1.0 to the queue.
        for (uint32_t j=0; j<i; j++) {                                            // Move over the 1.0 an additional position each time through the loop.
            queue_overwritePush(filter_getZQueue(filterNumber), 0.0);               // Move the 1.0 over by writing the correct number of zeros.
        }
        double iirValue = filter_iirFilter(filterNumber);                         // Run the IIR filter.
        double iirGoldenOutput =  filter_getIirACoefficientArray(filterNumber)[i+startingIndex];  // Golden output is simply the coefficient.
        if (!filterTest_floatingPointEqual(iirValue, -iirGoldenOutput)) {         // Check to see that the IIR-output matches the correct coefficient value.
            ...
            printf("filter_runIirAAlignmentTest: Output from IIR Filter[%d](%20.24le) does not match test-data(%20.24le) at index(%ld).\n\r", filterNumber, iirValue, -iirGoldenOutput, i+startingIndex);
        }
    }
    // Print informational messages.
    if (printMessageFlag) {
...
void filterTest_fillQueueWithRandomValues(queue_t* q) {
    for (uint16_t i=0; i<queue_size(q); i++)  // Iterate across all elements.
        queue_overwritePush(q, filterTest_randomValue0To1());  // Use rand to generate random values.
}

// Golden compute power function.
// Accepts a queue and computes the power value by computing the sum of the squares.
// Returns the computer power value.
double filterTest_computeGoldenPowerValue(queue_t* q) {
    double powerValue = 0.0;  // Result held here.
    // queueSizeOffset = 0 if output-queues are sized 2000, = 1 if you are storing an extra
    // value in the queue to hold the oldest value each time for a power computation.
    // You need to skip over that old value when computing the golden power value.
    queue_size_t queueSizeOffset = 0;
#ifdef FILTER_TEST_STORE_OLD_VALUE_IN_QUEUE
    queueSizeOffset = 1;
#endif
    for (uint16_t i=queueSizeOffset; i<queue_size(q); i++) {  // Iterate over all elements.
        double elementValue = queue_readElementAt(q, i);  // Read the value from queue.
...
#define TEST_INCREMENTAL_LOOP_COUNT 3000 // Loop over the incremental test this many times.
...
bool filterTest_runPowerTest() {
    ...
    for (uint16_t i=0; i<FILTER_FREQUENCY_COUNT; i++) {
        ...
        if ((size = queue_size(q)) != (OUTPUT_QUEUE_SIZE + queueSizeOffset)) {
            printf("Output queue for queue number %d is incorrect (%ld). It should be %ld\n\r", i, size, (OUTPUT_QUEUE_SIZE+queueSizeOffset));
            printf("Fix this problem before proceeding.\n\r");
            return false;
        }
        filterTest_fillQueueWithRandomValues(q);   // Fill the queue with random values.
        double goldenValue = filterTest_computeGoldenPowerValue(q); // Compute the golden value.
        // Compute power with the filter function.
        double testValue = filter_computePower(i, true, false);  // true, false = no force, no debug print.
        if (testValue != goldenValue) {  // Check for errors.
            printf("filter_runPowerTest failed for index: %d: , golden value: %lf, filter_computePower(): %lf\n\r",
    ...
    printf("Testing to see that the power is computed correctly incrementally over %d trials.\n\r",
           TEST_INCREMENTAL_LOOP_COUNT);
    bool incrementalComputeStatus = true; // Be optimistic for the incremental computation.
    // Perform many incrementat power computations.
    for (uint32_t loopCount=0; loopCount<TEST_INCREMENTAL_LOOP_COUNT; loopCount++) {
        for (uint16_t i=0; i<FILTER_FREQUENCY_COUNT; i++) {
            queue_t* q = filter_getIirOutputQueue(i);  // Get an output queue.
            queue_overwritePush(q, filterTest_randomValue0To1()); // Add a random value to every queue.
            double goldenValue = filterTest_computeGoldenPowerValue(q);   // Compute the golden value.
            double testValue = filter_computePower(i, false, false);  // false, false = no force, no debug print.
            if (fabs(testValue - goldenValue) > TEST_PASS_EPSILON) {  // See if the value is in error beyond some epsilon.
                printf("Loop count:%ld\n\r", loopCount);  // Print out the current loop count for reference.
                // Print out values that indicates the failure.
                printf("filter_runPowerTest failed for index: %d\n\rgolden value:          %lf\n\rfilter_computePower(): %20.24lf\n\r",
                       i, goldenValue, testValue);
                printf("Difference between golden value and incrementally computed value: %20.24le\n\r",
    ...
    // Return the combined status for both the first-compute test and the incremental test.
...
// 5. Plots the frequency response of each of the IIR bandpass filters on the TFT display.
// Returns true if all tests passed, false otherwise.
// Various informational prints are provided in the console during the run of the test.
...
bool filterTest_runTest() {
    ...
    // Confirm that the FIR coefficients are properly aligned with the incoming data.
    ...
    // Confirm that the IIR A coefficients are properly aligned with the incoming data.
    success &= filterTest_runIirAAlignmentTest(TEST_IIR_FILTER_NUMBER, PRINT_INFO_MESSAGES);
    // Confirm that the IIR B coefficients are properly aligned with the incoming data.
    success &= filterTest_runIirBAlignmentTest(TEST_IIR_FILTER_NUMBER, PRINT_INFO_MESSAGES);
