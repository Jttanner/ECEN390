...
bool queue_indexOutOfBounds(queue_t* q, queue_index_t index){
	if (queue_empty(q)){
		return true;
	}else if (q->indexIn > q->indexOut){
		if (index > q->indexIn - q->indexOut){
			return true;
		} else{
			return false;
		}
	} else{
		if (index > q->size - q->indexOut + q->indexIn){
			return true;
		} else{
			return false;
		}
